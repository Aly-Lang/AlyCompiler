...
Error typecheck_expression(ParsingContext* context, ParsingContext** context_to_enter, Node* expression, Node* result_type) {
    ...
    case NODE_TYPE_VARIABLE_ACCESS:
        ...
        if (err.type) {
            printf("Variable: \"%s\"\n", expression->value.symbol);
            ERROR_PREP(err, ERROR_GENERIC, "Could not get type definition in current context for variable access");
            return err;
        }
        *iterator = *tmpnode;
        *result_type = *type;
    ...
    case NODE_TYPE_ADDRESSOF:
        ...
        typecheck_expression(context, context_to_enter, expression->children, variable_access_return_type);
    ...
    case NODE_TYPE_BINARY_OPERATOR:
        ...
        err = parse_get_type(context, value->children->next_child->next_child, tmpnode);
        if (err.type) { return err; }
